...
namespace DoveAutomation.Utilities
{
    [Binding]
    public class BaseClass
    {
        ...
        {
            var htmlReport = new ExtentHtmlReporter(@"C:\Users\DELL\source\repos\DoveAutomation\DoveAutomation\My Report\index.html");
            ...
            Log.Logger = new LoggerConfiguration().MinimumLevel.ControlledBy(levelSwitch: loggingLevel).WriteTo.File(
                @"C:\Users\DELL\source\repos\DoveAutomation\DoveAutomation\My Report\Logs", outputTemplate: "{TimeSpamp:yyyy-MM-dd HH:mm:ss.fff} | {Level:u3} | {Message}{NewLine}", rollingInterval: RollingInterval.Day).CreateLogger();
        }
        [AfterTestRun]
